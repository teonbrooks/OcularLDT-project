...
for subject in subjects:
    rep = Report()
    subject_template = op.join('%s', subject, 'mne', subject + '_%s_%s.%s')
    fname_gat = subject_template % (path, exp, 'calm_' + filt + '_filt_' +
                                    analysis + '_gat', 'npy')
    fname_reg = subject_template % (path, exp, 'calm_' + filt + '_filt_' +
                                    analysis + '_reg-ave', 'fif')
    group_gat[subject] = np.load(fname_gat)
    group_reg[subject] = mne.read_evokeds(fname_reg)


################
# Group Evoked #
################
grand_averages = [mne.grand_average([group_reg[subject][0] for
                                     subject in subjects])]
fig = grand_averages[0].plot()
group_rep.add_figs_to_section(fig, 'Grand Average Regression', 'Group Plots')
...
fig = ax.get_figure()
ax.set_title('Group GAT scores on Processing FFD')
...
fig = ax.get_figure()
ax.set_title('Group TD scores on Processing FFD')
