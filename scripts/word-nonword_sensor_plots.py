...
group_template = op.join(config.results_dir, 'group',
                         'group_OLDT_%s_filt_%s.%s')
fname_group_rep = group_template % (filt, analysis, 'html')
fname_group_rerf = group_template % (filt, 'rerf', 'mne')
fname_group_gat = group_template % (filt, 'gat', 'mne')
...
# Group Evoked #
################
group_word = mne.grand_average([group_rerf[subject]['word'] for subject
                                in subjects])
group_nonword = mne.grand_average([group_rerf[subject]['nonword'] for subject
                                   in subjects])
condition_names = ['word', 'nonword']
