...
ica_file = op.join(path, raw_file[:-7] + 'ica.fif')
cleaned_raw = op.join(path, raw_file[:-7] + 'cleaned-raw.fif')
...
report.add_figs_to_section(p, 'Covariance Matrix', 'Summary', image_format='svg')
...
r.add_figs_to_section(p, 'PCA topographies', 'Summary', image_format='png')
...
# report.add_figs_to_section(p, 'ICs over time', 'Summary', image_format='svg')
#
#
# evokeds = list()
